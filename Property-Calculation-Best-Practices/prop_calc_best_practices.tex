...
%is perfect. Corrections from feedback cannot be made instantaneously, hence 
...
\[\Delta G_{ij}(T) = k_B T \left (\Delta f_{ij}(T) - \Delta f_{ij}(T_{ref})\right) + \frac{T}{T_{ref}}\Delta G_{ij}(T_{ref})\], 
...
\rangle$, $\langle A(x+\Delta x) \rangle$, and $\langle A(x-\Delta x) \rangle$, meaning in practice
...
$C_V = \frac{\partial U}{\partial T}_V$, and isobaric heat capacity
$C_P = \frac{\partial H}{\partial T}_P$. The are calculated 

\paragraph{Direct simulation estimate}
There is no direct way to estimate either heat capacity.

\paragraph
The definition of the isobaric heat capacity is:
\begin{equation}C_P = \left( \frac{\partial H}{\partial T}\right)_{P,N}\end{equation}
%MRS: probably easier to take df/dbeta, and then take the temperature derivative, which can be converted into a second beta derivative by a change of variable, so it's a 2nd derivative of f WRT beta, with some prefactors of beta.
...
\begin{equation}\kappa_T = \beta \frac{\langle \Delta V^2 \rangle_{NTP}}{\langle V \rangle_{NTP}} = V \beta \frac{\langle \Delta N^2 \rangle_{VT}}{\langle N \rangle_{\mu VT}}\end{equation}\\*
