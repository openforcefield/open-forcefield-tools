...
phase = np.pi/3.
def fourier(x, *a):
    ret =  a[0] * (np.sin(2*np.pi*x / a[1] + 2.0))**2 + \
           a[2] * (np.sin(2*np.pi*x / a[3] + 2.0))**2 + \
           a[4] * (np.sin(2*np.pi*x / a[5] + 2.0))**2 # \
           #[9]
...
popt, pcov = sci.curve_fit(fourier, bins1[1:], n1, [5.0,3.0]*3 , maxfev=100000)
